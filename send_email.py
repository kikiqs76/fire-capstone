...
def extract_summary(file_path="data_input/data.csv", id=['936', '1178']):
    ...
    campaigns['reporting_start'] = pd.to_datetime(
        campaigns['reporting_start'], dayfirst=True)
    ...
    cpc = campaigns.groupby(['campaign_id'])[
        ["spent", "total_conversion"]].sum()
...
def compose_email(template, name, data_dict):
    ...
    composed = template.substitute(
        ...
        GITHUB_LINK='https://github.com/kikiqs76/fire-capstone'
    )
    return composed
...
def create_plot(file_path="data_input/data.csv", id=['936', '1178']):
    ...
    for i, campaign in enumerate(grouped.campaign_id.unique()):
        ...
        pivot = df.pivot(index='reporting_start',
                         columns='age', values='total_conversion').fillna(0)
